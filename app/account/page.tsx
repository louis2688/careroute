...
function SignIn({ error }: { error?: string }) {
  return (
    <div className="mx-auto max-w-md px-4 py-16 sm:px-6">
      <p className="text-sm font-semibold tracking-wide text-sky-700 uppercase dark:text-sky-400">Passengers</p>
      <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">My rides</h1>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:py-16">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold tracking-wide text-sky-700 uppercase dark:text-sky-400">My rides</p>
          <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
            Hi {user.name?.split(" ")[0] || "there"}
          </h1>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Rides booked with {user.id}.</p>
        </div>
        ...
        <ul className="mt-8 space-y-3">
          {rides.map((r) => (
            <li key={r.id} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
              ...
              <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{r.pickup}</p>
              <p className="text-sm text-slate-500 dark:text-slate-400">to {r.dropoff}</p>
              <div className="mt-3 flex flex-wrap items-center gap-4 text-sm">
                <Link href={`/trip/${r.ref}`} className="font-medium text-sky-700 hover:underline dark:text-sky-400">
                  Track {r.ref}
                </Link>
