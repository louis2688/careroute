...
const input =
  "mt-1.5 block w-full rounded-lg border border-slate-300 bg-white px-3 py-3 text-base text-slate-900 focus:border-sky-700";
...
function Login({ error }: { error: boolean }) {
  return (
    <div className="mx-auto max-w-sm px-4 py-12 sm:px-6">
      <p className="text-sm font-semibold tracking-wide text-sky-700 uppercase">Driver app</p>
      <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-slate-900">Sign in</h1>
      <p className="mt-2 text-slate-600">Use the phone number dispatch has on file and your PIN.</p>
      {error && (
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold tracking-wide text-sky-700 uppercase">Driver app</p>
          <h1 className="mt-1 font-heading text-2xl font-bold tracking-tight text-slate-900">{driver.name}</h1>
        ...
        {rides.map((r) => {
          ...
          return (
            <li key={r.id} className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <a href={`tel:${r.phone}`} className="mt-1 inline-flex min-h-10 items-center gap-2 text-sky-700 underline">
                <Icon name="phone" className="size-4" />
                {r.phone}
              </a>
              <dl className="mt-3 space-y-2 text-sm">
                <div>
                  <dt className="font-medium text-slate-700">Pickup</dt>
                  <dd>
                    <a href={maps(r.pickup)} target="_blank" rel="noreferrer" className="text-sky-700 underline">
                      {r.pickup}
                    </a>
                  </dd>
                </div>
                <div>
                  <dt className="font-medium text-slate-700">Destination</dt>
                  <dd>
                    <a href={maps(r.dropoff)} target="_blank" rel="noreferrer" className="text-sky-700 underline">
                      {r.dropoff}
                    </a>
                  </dd>
                </div>
              ...
              {next && (
                <form action={driverUpdate} className="mt-4 space-y-3">
                  ...
                  {next.status === "completed" && (
                    <div>
                      <p className="text-sm font-medium text-slate-800">Passenger signature</p>
