...
export function HomePage({ lang }: { lang: Lang }) {
  ...
  return (
    <>
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:py-24">
          <div>
            ...
            <h1 className="mt-3 font-heading text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">{t.h1}</h1>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 sm:p-8">
            <h2 className="font-heading text-lg font-semibold text-slate-900">{t.includes}</h2>
      ...
      <section aria-label={t.credentials} className="bg-slate-900 text-white">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            ...
            <h3 className="mt-8 font-heading font-semibold text-slate-900">{t.whoWeServe}</h3>
            <ul className="mt-3 flex flex-wrap gap-2">
              {site.serves.map((s) => (
                <li key={s} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700">
                  {s}
                </li>
              ))}
            </ul>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 lg:self-start">
            <div className={card}>
              <h3 className="font-heading text-lg font-semibold text-slate-900">{t.mission}</h3>
              <p className="mt-2 text-slate-600">{site.mission}</p>
            </div>
            <div className={card}>
              <h3 className="font-heading text-lg font-semibold text-slate-900">{t.vision}</h3>
