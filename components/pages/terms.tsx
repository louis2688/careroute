    ...
    <>
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[16rem_1fr] lg:py-16">
        <nav aria-label={t.onThisPage} className="lg:sticky lg:top-24 lg:self-start">
          <p className="text-sm font-semibold text-slate-900">{t.onThisPage}</p>
          <ol className="mt-3 space-y-1 text-sm">
            {site.terms.map((s, i) => (
              <li key={s.title}>
                <a href={`#${slug(s.title)}`} className="inline-block py-1 text-slate-600 transition-colors hover:text-sky-700">
                  {i + 1}. {s.title}
                </a>
              </li>
            ))}
